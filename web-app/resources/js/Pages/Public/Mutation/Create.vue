<template>
  <Head :title="$t('public.mutations.createTitle')"/>
  <v-container fluid>
    <flash-messages/>
    <v-form @submit.prevent="submit">
      <v-row>
        <v-col cols="12" md="4">
          <div :class="['text-h5', 'pa-2']">{{ $t('public.mutations.createTitle') }}</div>
        </v-col>
      </v-row>
      <v-row>
        <v-col cols="12" md="4">
          <v-text-field
            v-model="form.title"
            :label="$t('public.mutations.fields.title')"
            :rules="rules.required"
          ></v-text-field>
          <v-textarea
            v-model="form.remarks"
            :label="$t('public.mutations.fields.remark')"
          ></v-textarea>
        </v-col>
      </v-row>
      <v-row>
...
async function submit(event) {
  ...
  if (results.valid) {
    form.post(route('public.vegetation.mutation.store', {
